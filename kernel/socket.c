...
void sock_dns_found(const char *name, const ip4_addr_t *ipaddr, void *callback_arg)
{
    ip4_addr_t *addr = (ip4_addr_t *)callback_arg;
...
int sockconnect(int sockfd, const struct sockaddr *addr, int addrlen) 
{
    ...
    ip4_addr_t ipaddr = {addr->sin_addr};
...
int sockbind(int sockfd, const struct sockaddr *addr, int addrlen)
{
    ...
    err_t err = tcp_bind(sock->pcb, &ipaddr,ntohs(addr->sin_port));
    printf("sockbind: tcp_bind returned %d\n", err);
    if (err == ERR_USE) {
    ...
    printf("sockbind: bound to IP %x, port %d\n", ipaddr.addr, ntohs(addr->sin_port));
...
int socklisten(int sockfd, int backlog)
{
    ...
    printf("socklisten: PCB local_ip=%x, local_port=%d, state=%d\n",
    sock->pcb->local_ip.addr, sock->pcb->local_port, sock->pcb->state);
    sock_setup_callbacks_accept(sock);
    printf("socklisten: tcp_accept callback set on PCB %p\n", sock->pcb);
    return 0;
}
...
int sockaccept(int sockfd, struct sockaddr *addr, int *addrlen)
{
    ...
    printf("sockaccept: addr set to %s:%d\n", inet_ntoa(sock->accept_pcb->remote_ip), ntohs(addr->sin_port));
    // reset temporary fields in the socket
    sock->accept_pcb = NULL;
    sock->accept_fd = -1;
    printf("sockaccept: reset sock->accept_pcb and sock->accept_fd\n");
